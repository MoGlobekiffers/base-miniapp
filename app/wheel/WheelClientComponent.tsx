...
export default function WheelClientPage() { 
  ...
  const handleSpin = () => {
    ...
    setTimeout(() => {
      ...
      if (address && !DEV_MODE && !COMING_SOON_QUESTS.includes(q) && q !== "Bonus spin") localStorage.setItem(`dw:lastSpin:${address.toLowerCase()}`, String(Date.now()));
      setSpinning(false);
    }, SPIN_DURATION_MS);
  };
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-50 flex flex-col items-center pt-2 px-2 overflow-x-hidden relative">
      ...
      {activeQuiz && !quizResult && (
        <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in zoom-in-95">
          <div className="bg-slate-900 border-2 border-blue-500 rounded-2xl p-6 max-w-sm w-full shadow-lg">
            ...
            <h3 className="text-xl font-black text-white mb-8 text-center">{activeQuiz.question}</h3>
      ...
      <div className="relative w-full max-w-[360px] aspect-square md:max-w-[500px] mb-8">
        <svg viewBox="-300 -300 600 600" className="w-full h-full drop-shadow-2xl"><circle r={R_OUT+12} fill="#0f172a"/><circle r={R_OUT+8} fill="none" stroke="#1e293b" strokeWidth={4}/><g style={{transform:`rotate(${rotation}deg)`,transformOrigin:"center",transition:`transform ${SPIN_DURATION_MS}ms cubic-bezier(0.2,0.8,0.2,1)`}}>{segments.map((s)=><path key={`w-${s.i}`} d={wedgePath(R_OUT,R_IN,s.a0,s.a1)} fill={s.color} stroke="#0f172a" strokeWidth={2}/>)}{segments.map((s)=><g key={`l-${s.i}`} transform={`rotate(${s.a0+anglePerSegment/2}) translate(0, -${(R_OUT+R_IN)/2}) rotate(90)`}><text textAnchor="middle" dominantBaseline="middle" fill="#2e1065" fontSize="13" fontWeight={900}>{s.label}</text></g>)}</g><div className="absolute left-1/2 -translate-x-1/2 -translate-y-full" style={{top: -20, left: 0}}><svg width="50" height="40" viewBox="0 0 50 40"><path d="M25 40 L10 10 H40 Z" fill="#3b82f6" stroke="#cffafe" strokeWidth={2}/></svg></div></svg>
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none"><button onClick={handleSpin} disabled={!(!spinning&&(!cooldown||DEV_MODE))} className="pointer-events-auto w-28 h-28 rounded-full border-4 border-blue-500 overflow-hidden relative"><img src="/base-logo-in-blue.png" className="absolute inset-0 w-full h-full object-cover"/><span className="relative z-10 text-2xl font-black text-white">SPIN</span></button></div>
      </div>
